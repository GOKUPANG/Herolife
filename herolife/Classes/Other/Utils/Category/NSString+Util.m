...
+ (NSString *)stringWithSocketLogOut;
{
    ...
    //从偏好设置里 取token
    hrpushDict[@"token"] = token;
    hrpushDict[@"type"] = @"logout";
    hrpushDict[@"desc"] = @"login";
    hrpushDict[@"src"] = @"huaruicloud";//
    hrpushDict[@"dst"] = @"huaruiPushServer";//
    
    NSMutableDictionary *msgDict = [NSMutableDictionary dictionary];
    msgDict[@"user"] = user;
    msgDict[@"pass"] = [kUserDefault objectForKey:kDefaultsPassWord];
...
+ (NSString *)stringWithUDPMsgDict:(NSMutableDictionary *)msgDict
{
    ...
    for (int i = 0; i < dictStr.length; i++) {
        NSString *stringM = [dictStr substringWithRange:NSMakeRange(i, 1)];
        BOOL isChinese = [[[self alloc] init] isChinese:stringM];
        if (isChinese) {
            index++;
        }
    }
    ...
    NSString *hrlength = [NSString stringWithFormat:@"length\r\n%lu\r\n", (unsigned long)dictStr.length - index + index *3];
